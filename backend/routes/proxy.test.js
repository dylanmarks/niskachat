...
describe("FHIR Proxy Routes", () => {
  ...
  describe("Request Validation", () => {
    ...
    it("should reject dangerous path parameters", async () => {
      const response = await request(app).get("/proxy/fhir/../etc/passwd?sessionId=test");
    ...
    it("should reject paths with backslashes", async () => {
      const response = await request(app).get("/proxy/fhir/..\\windows\\system32?sessionId=test");
    ...
    it("should reject paths with tilde", async () => {
      const response = await request(app).get("/proxy/fhir/~/secrets?sessionId=test");
  ...
  describe("Authentication", () => {
    it("should require valid authentication", async () => {
      ...
      const response = await request(app).get("/proxy/fhir/Patient?sessionId=test");
      
      expect(response.status).toBe(401);
      expect(response.body.error).toBe("Authentication required or token expired");
    });
  });
  ...
  describe("HTTP Methods", () => {
    ...
    it("should handle POST requests", async () => {
      const requestBody = { resourceType: "Patient", name: [{ family: "Doe" }] };
    ...
    it("should handle DELETE requests", async () => {
      const response = await request(app).delete("/proxy/fhir/Patient/123?sessionId=test");
    ...
    it("should handle PATCH requests", async () => {
      const patchBody = [{ op: "replace", path: "/name/0/family", value: "Smith" }];
  ...
  describe("Path Handling", () => {
    ...
    it("should handle resource with ID paths", async () => {
      const response = await request(app).get("/proxy/fhir/Patient/123?sessionId=test");
      
      expect(response.status).toBe(401); // Auth error, not path error
    });

    it("should handle complex nested paths", async () => {
      const response = await request(app).get("/proxy/fhir/Patient/123/Observation?sessionId=test");
      
      expect(response.status).toBe(401); // Auth error, not path error
    });

    it("should handle paths with search parameters", async () => {
      const response = await request(app).get("/proxy/fhir/Patient?sessionId=test&_count=10&family=Doe");
