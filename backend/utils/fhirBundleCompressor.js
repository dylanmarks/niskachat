...
/**
 * Compress a FHIR Bundle for header display (basic patient info only)
 * @param {Object} bundle - FHIR R4-compliant Bundle (JSON object)
 * @returns {string} - Basic patient info for header display
 */
export function compressFHIRBundleForHeader(bundle) {
  if (!bundle || !bundle.entry || !Array.isArray(bundle.entry)) {
    return "Invalid or empty FHIR Bundle";
  }
  ...
  // Only return basic patient info for header display
  if (resourceGroups.Patient) {
    return compressPatient(resourceGroups.Patient[0]); // Assume single patient
  }
...
function getPatientMRN(patient) {
  ...
  const mrnIdentifier = patient.identifier.find(
    (id) =>
      ...
      id.type.coding.some(
        (coding) =>
          coding.code === "MR" || coding.display === "Medical record number",
      ),
  );
  ...
  const activeAllergies = allergies.filter(
    (allergy) =>
      ...
      (allergy.clinicalStatus.coding &&
        allergy.clinicalStatus.coding.some(
          (coding) => coding.code === "active",
        )),
  );
