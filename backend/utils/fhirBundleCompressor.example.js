...
console.log(
  `- Resource types: ${[...new Set(exampleBundle.entry.map((e) => e.resource.resourceType))].join(", ")}`,
);
console.log();
...
console.log(
  `- Original JSON size: ${JSON.stringify(exampleBundle).length} characters`,
);
console.log(`- Compressed size: ${compressed.length} characters`);
console.log(
  `- Compression ratio: ${Math.round((1 - compressed.length / JSON.stringify(exampleBundle).length) * 100)}%`,
);
...
export { compressFHIRBundle, exampleBundle };
