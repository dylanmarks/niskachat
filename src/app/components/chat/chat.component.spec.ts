...
import { ChatComponent, ChatMessage, ChatRequest, ChatResponse } from './chat.component';
...
describe('ChatComponent', () => {
  ...
  beforeEach(async () => {
    ...
    fhirClientService = TestBed.inject(FhirClientService) as jasmine.SpyObj<FhirClientService>;
  });
  ...
  describe('Component Initialization', () => {
    ...
    it('should initialize with welcome message', () => {
      ...
      expect(component.messages[0].content).toContain('Hello! I\'m your clinical AI assistant');
    });
  ...
  describe('Message Management', () => {
    ...
    it('should update loading message correctly', () => {
      const messageId = component['addLoadingMessage']();
      const messageIndex = component.messages.findIndex(m => m.id === messageId);
  ...
  describe('Message Sending', () => {
    beforeEach(() => {
      ...
      fhirClientService.getCurrentContext.and.returnValue({
        authenticated: true,
        patient: { id: 'test-patient', name: [{ given: ['John'], family: 'Doe' }] }
      });
    });
    ...
    it('should send message successfully', async () => {
      ...
      expect(component.messages[initialCount].content).toBe('What are the patient conditions?');
      expect(component.messages[initialCount].isUser).toBe(true);
      expect(component.messages[initialCount + 1].content).toBe('Patient has hypertension and diabetes.');
    ...
    it('should handle server error', async () => {
      ...
      expect(component.messages[initialCount + 1].content).toBe('Server error occurred. Please try again later.');
      expect(component.isLoading).toBe(false);
    });
    ...
    it('should handle network error', async () => {
      ...
      // Verify error message
      expect(component.messages.length).toBe(initialCount + 2);
      expect(component.messages[initialCount + 1].content).toBe('Unable to connect to the server. Please check your connection.');
      expect(component.isLoading).toBe(false);
    });
  ...
  describe('Chat Controls', () => {
    it('should clear chat correctly', () => {
      ...
      expect(component.messages.length).toBe(1);
      expect(component.messages[0].content).toBe('Chat cleared. How can I help you analyze the patient data?');
      expect(component.messages[0].isUser).toBe(false);
    });
